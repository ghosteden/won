...
function filesIsOk() {
	$('#checkUpdateApps').fadeOut(function() { // on masque
		$('#checkUpdateApps').html(lang('loading')).fadeIn(function() {
			getLocalData('ressources/langFile', function() {
				for(var language in globalVars['langFile']){
					for(var value in globalVars['langFile'].LANG_FR){
						LANG[language][value] = globalVars['langFile'][language][value];
					}
				}
